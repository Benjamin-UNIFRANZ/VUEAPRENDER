<template>
  <div class="container-fluid editor-container">
    <!-- Editor y panel de proyecto en dos columnas -->
    <div class="row mb-3">
      <div class="col-md-8">
        <div class="card editor-card">
          <div class="card-header editor-header d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
              <h5 class="mb-0 me-3 editor-title">Editor C#</h5>
              <div class="input-group" style="max-width: 400px;">
                <input 
                  type="text" 
                  class="form-control form-control-sm file-name-input"
            ...
            <div class="d-flex gap-2">
              ...
              <button v-if="sharedFileId" @click="showSaveToProjectModal" class="btn btn-success btn-sm">
                Guardar en Proyecto
              </button>
            </div>
          </div>
          <div class="card-body editor-body position-relative">
            <!-- Botón para copiar código -->
            <button 
              @click="copyCodeToClipboard" 
              class="btn btn-sm btn-light position-absolute top-0 end-0 m-2 copy-button" 
              title="Copiar código"
            >
      ...
      <div class="col-md-4">
        ...
        <div class="card project-panel-card">
          <div class="card-header project-panel-header bg-dark text-white">
            <h5 class="mb-0 project-panel-title">Proyecto</h5>
          </div>
          <div class="card-body project-panel-body">
            <!-- Botón para ver todos los proyectos -->
            <div class="mb-3">
              <button @click="goToProjects" class="btn btn-primary btn-block w-100">
                <i class="bi bi-folder"></i> Ver todos mis proyectos
              </button>
            </div>
            
            <div class="mb-3">
              <label class="form-label">Abrir Proyecto</label>
            ...
            <div v-if="currentProject">
              ...
              <div class="mb-3">
                <label for="newFileName" class="form-label">Nuevo Archivo</label>
              ...
              <ul class="list-group">
                <li v-for="file in projectFiles" :key="file.id" class="list-group-item d-flex justify-content-between align-items-center">
                  {{ file.name }}
                  <div class="btn-group">
            ...
            <div v-else>
              <p>Selecciona un proyecto para ver sus archivos.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="modal fade" id="newProjectModal" tabindex="-1" ref="newProjectModalRef">
      <div class="modal-dialog">
        <div class="modal-content new-project-modal">
          <div class="modal-header new-project-header">
    ...
    <!-- JDoodle iframe a ancho completo -->
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0">JDoodle C# Online</h5>
            <div>
              <button @click="toggleFullScreen" class="btn btn-sm btn-light me-2">
                <i :class="jdoodleFullScreen ? 'bi bi-fullscreen-exit' : 'bi bi-fullscreen'"></i>
                {{ jdoodleFullScreen ? 'Reducir' : 'Pantalla completa' }}
              </button>
              <button @click="reloadJdoodle" class="btn btn-sm btn-light">
                <i class="bi bi-arrow-clockwise"></i> Recargar
              </button>
            </div>
          </div>
          <div class="card-body p-0 mb-5" :style="{ height: jdoodleFullScreen ? '80vh' : '1000px' }">
            <div class="alert alert-info m-2" v-if="showJdoodleHelp">
              <h5>¿Cómo usar JDoodle?</h5>
              <ol>
              ...
              <button @click="showJdoodleHelp = false" class="btn btn-sm btn-primary">Entendido</button>
            </div>
            <iframe 
              ref="jdoodleFrame" 
    ...
    <div class="modal fade" id="saveToProjectModal" tabindex="-1" ref="saveToProjectModalRef">
      <div class="modal-dialog">
        <div class="modal-content save-to-project-modal">
          <div class="modal-header save-to-project-header">
            <h5 class="modal-title">Guardar en Proyecto</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body save-to-project-body">
            <p>Selecciona un espacio de proyecto para guardar el archivo:</p>
            <select class="form-select" v-model="selectedProjectSpace">
              <option v-for="space in projectSpaces" :key="space.id" :value="space.id">
                {{ space.name }}
              </option>
            </select>
          </div>
          <div class="modal-footer save-to-project-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
              Cancelar
            </button>
